...
runners.forEach(function(element){
  eachRunner.push(element.first_name+' '+element.last_name+' works at ' + 
  element.company_name +' and has donated '+element.donation)
});
...
let sizeExpand = []
sizeExpand = runners.map(function(element) {
  return element.first_name+" "+element.last_name+'\'s email id is ' + element.email
  });
console.log(sizeExpand);

// Problem 3

let filterSize = []
filterSize = runners.map(element => element).
filter(element => element.shirt_size === 'M').
reduce((accumulator,curValue) => accumulator+curValue.donation,0)
